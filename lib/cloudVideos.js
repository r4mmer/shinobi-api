...
class CloudVideos extends Section {
  getAll() {
    return this.axios.get(`/${this.s.$user.auth_token}/cloudVideos/${this.s.$user.ke}`).then(r => r.data);
  }
  ...
  getForMonitor(monitorID, params) {
    ...
    params = params || {};
    return this.axios.get(`/${this.s.$user.auth_token}/cloudVideos/${this.s.$user.ke}/${monitorID}`, { params }).then(r => r.data);
  }

  fileURL(monitorID, filename) {
    // Should this be a promise? to keep the same usage as other functions? or is it unnecessary?
    return `/${this.s.$user.auth_token}/cloudVideos/${this.s.$user.ke}/${monitorID}/${filename}`;
  }
