...
class Videos extends Section {
  ...
  getAll() {
    return this.axios.get(`/${this.s.$user.token}/videos/${this.s.$user.ke}`).then(r => r.data);
  }
  ...
  getForMonitor(monitorID, params) {
    ...
    params = params || {};
    return this.axios.get(`/${this.s.$user.token}/videos/${this.s.$user.ke}/${monitorID}`, { params }).then(r => r.data);
  }

  fileURL(monitorID, filename) {
    // Should this be a promise? to keep the same usage as other functions? or is it unnecessary?
    return `/${this.s.$user.token}/videos/${this.s.$user.ke}/${monitorID}/${filename}`;
  }
  ...
  getZipped(videoArr) {
    ...
    return this.axios.get(`/${this.s.$user.token}/zipVideos/${this.s.$user.ke}`, { params: { videos: videoArr } });
  }

  // Docs: https://shinobi.video/docs/api#content-modifying-a-video-or-deleting-it
  del(monitorID, filename) {
    return this.axios.get(`/${this.s.$user.token}/videos/${this.s.$user.ke}/${monitorID}/${filename}/delete`).then(r => r.data);
  }
  markUnread(monitorID, filename) {
    return this.axios.get(`/${this.s.$user.token}/videos/${this.s.$user.ke}/${monitorID}/${filename}/status/1`).then(r => r.data);
  }
  markRead(monitorID, filename) {
    return this.axios.get(`/${this.s.$user.token}/videos/${this.s.$user.ke}/${monitorID}/${filename}/status/2`).then(r => r.data);
  }
}
